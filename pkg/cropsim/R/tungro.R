...
# R translation: Robert J. Hijmans , Rene Pangga, & Jorrel Aunario r.hijmans@gmail.com (translated from STELLA TungroMod v5)
...
Incidence[day] <- (Diseased[day]-Removed[day])/(TotalSites - Removed[day])*100	
	...
	res <- cbind(Sites, now_latent, now_infectious, Removed, Diseased, Incidence)
	res <- res[1:day,]
	#res <- Diseased / AllSites 
	res <- cbind(1:length(res[,1]), res)
	colnames(res) <- c("day", "sites", "latent", "infectious", "removed", "diseased", "incidence")
	return(res)
}
