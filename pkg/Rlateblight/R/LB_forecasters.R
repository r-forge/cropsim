...
Hyre <- function(wth, emergence='2000-05-15', duration=120) {
	...
	t5 <- .movingFun(w$tavg, n=5, type='to', na.rm=TRUE) / 5
	p10 <- .movingFun(w$prec, n=10, type='to', na.rm=TRUE)
...
blitecast <- function(wth, emergence='2000-05-15', duration=120) {
	...
	rainfav7 <- .movingFun(wth@w$prec > 3, n=7, type='to', na.rm=TRUE)
	severity7 <- .movingFun(wallin$severity, n=7, type='to', na.rm=TRUE)
